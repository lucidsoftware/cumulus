  ...
  module S3
    class Manager < Common::Manager
      ...
      def create(local)
        ...
        update_logging(local.name, local.logging)
        update_website(local.name, local.website)
        update_lifecycle(local.name, local.lifecycle)
        update_notifications(local.name, local.notifications)
        update_replication(local.name, local.replication)
        update_tags(local.name, local.tags)
      end
